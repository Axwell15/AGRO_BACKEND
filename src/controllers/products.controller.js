...
export const rateProductsInCart = async (req, res) => {
    try {
        const authToken = req.headers.authorization && req.headers.authorization.split(" ")[1];
        const { id } = jwt.verify(authToken, process.env.SECRET_KEY);
        const { ratings } = req.body; // Expecting an array of { productId, rating }

        if (!Array.isArray(ratings) || ratings.length === 0) {
            return res.status(400).json({ error: "Debe proporcionar una lista de calificaciones" });
        }

        for (const { productId, rating } of ratings) {
            ...
            const existingRating = product.ratings.find(r => r.user.toString() === id);

            if (existingRating) {
                existingRating.rating = rating;
            } else {
                product.ratings.push({ user: id, rating });
            }
        ...
        res.status(200).json({ message: "Calificaciones actualizadas" });
    } catch (error) {
        console.log(error);
        res.status(500).json({ error: "Error de server" });
    }
